...
import { createDOMRenderer, makeResetStyles, makeStyles } from '../index.js';
...
export const commands = rawCommands;
...
export function getComputedBackgroundColor(el) {
    return getComputedStyle(el).backgroundColor;
}
export function getColor(el) {
    return getComputedStyle(el).color;
}
export function applyStyles(stylesBySlot) {
    const getStyles = makeStyles(stylesBySlot);
    const renderer = createDOMRenderer(document);
    return getStyles({ dir: 'ltr', renderer });
}
export function applyResetStyles(styles) {
 ...
 * renderer cache.
 */
export function resetBrowserTestState() {
