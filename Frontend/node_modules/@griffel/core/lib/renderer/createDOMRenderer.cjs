...
function createDOMRenderer(targetDocument = typeof document === 'undefined' ? undefined : document, options = {}) {
    const { classNameHashSalt, unstable_filterCSSRule, insertionPoint, styleElementAttributes, compareMediaQueries = defaultCompareMediaQueries, // Container queries default to the same comparator as media queries; pass `compareCSSQueries`
    // from `@griffel/sort-css-queries` for numeric min-width / max-width ordering.
    compareContainerQueries = compareMediaQueries } = options;
    const renderer = {
        classNameHashSalt,
        insertionCache: {},
        stylesheets: {},
