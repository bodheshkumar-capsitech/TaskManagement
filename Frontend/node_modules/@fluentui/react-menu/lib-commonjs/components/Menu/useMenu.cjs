...
const _useMenuItemStylesstyles = require("../MenuItem/useMenuItemStyles.styles.cjs");
const _MenuSurfaceMotion = require("./MenuSurfaceMotion.cjs");
...
const useMenuBase_unstable = (props)=>{
    ...
    const safeZoneHandle = (0, _reactpositioning.useSafeZoneArea)({
        ...
        onSafeZoneTimeout: ()=>{
            const event = new CustomEvent(_index.MENU_SAFEZONE_TIMEOUT_EVENT);
            setOpen(event, {
                open: false,
                keyboard: false,
...
const useMenuOpenState = (state)=>{
    ...
    const setOpen = (0, _reactutilities.useEventCallback)((e, data)=>{
        ...
        const shouldUseDelay = !data.ignoreHoverDelay && (e.type === 'mouseleave' || e.type === 'mouseover' || e.type === 'mousemove' || e.type === _index.MENU_ENTER_EVENT);
        if (shouldUseDelay) {
