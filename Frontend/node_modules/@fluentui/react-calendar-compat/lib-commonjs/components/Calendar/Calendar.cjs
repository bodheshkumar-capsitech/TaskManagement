...
const _CalendarDay = require("../CalendarDay/CalendarDay.cjs");
const _CalendarMonth = require("../CalendarMonth/CalendarMonth.cjs");
const _calendarNavigationIcons = require("./calendarNavigationIcons.cjs");
const _useCalendarStylesstyles = require("./useCalendarStyles.styles.cjs");
const MIN_SIZE_FORCE_OVERLAY = 440;
const defaultWorkWeekDays = [
    _index.DayOfWeek.Monday,
    _index.DayOfWeek.Tuesday,
    _index.DayOfWeek.Wednesday,
    _index.DayOfWeek.Thursday,
    _index.DayOfWeek.Friday
];
...
function useFocusLogic({ componentRef }, isDayPickerVisible, isMonthPickerVisible) {
    ...
    const focus = _react.useCallback(()=>{
        if (isDayPickerVisible && dayPicker.current) {
            (0, _index.focusAsync)(dayPicker.current, win);
        } else if (isMonthPickerVisible && monthPicker.current) {
            (0, _index.focusAsync)(monthPicker.current, win);
        }
    }, [
        isDayPickerVisible,
        isMonthPickerVisible,
...
const Calendar = /*#__PURE__*/ _react.forwardRef((props, forwardedRef)=>{
    const { allFocusable = false, calendarDayProps, calendarMonthProps, className, componentRef, dateRangeType = _index.DateRangeType.Day, dateTimeFormatter = _index.DEFAULT_DATE_FORMATTING, firstDayOfWeek = _index.DayOfWeek.Sunday, firstWeekOfYear = _index.FirstWeekOfYear.FirstDay, highlightCurrentMonth = false, highlightSelectedMonth = false, id, isDayPickerVisible: isDayPickerVisibleProp = true, isMonthPickerVisible: isMonthPickerVisibleProp = true, maxDate, minDate, onDismiss, onSelectDate, restrictedDates, showCloseButton = false, showGoToToday = true, showMonthPickerAsOverlay: showMonthPickerAsOverlayProp = false, showSixWeeksByDefault = false, showWeekNumbers = false, strings = _index.DEFAULT_CALENDAR_STRINGS, today: todayProp, value, workWeekDays = defaultWorkWeekDays } = props;
    ...
    const onDatePickerPopupKeyDown = (ev)=>{
        switch(ev.key){
            ...
            case _keyboardkeys.PageUp:
                if (ev.ctrlKey) {
                    // go to next year
                    navigateDay((0, _index.addYears)(navigatedDay, 1));
                } else {
                    // go to next month
                    navigateDay((0, _index.addMonths)(navigatedDay, 1));
                }
                ev.preventDefault();
                break;
