...
const _useCalendarDayGridStylesstyles = require("./useCalendarDayGridStyles.styles.cjs");
const CalendarGridDayCell = (props)=>{
    ...
    const navigateMonthEdge = (ev, date)=>{
        ...
        let nextDate = (0, _index.findAvailableDate)(findAvailableDateOptions);
        if (!nextDate) {
            // if no dates available in initial direction, try going backwards
            findAvailableDateOptions.direction = -direction;
            nextDate = (0, _index.findAvailableDate)(findAvailableDateOptions);
        }
        ...
        const isInCurrentView = weeks && nextDate && weeks.slice(1, weeks.length - 1).some((week)=>{
            return week.some((dayToCompare)=>{
                return (0, _index.compareDates)(dayToCompare.originalDate, nextDate);
            });
        });
        if (isInCurrentView) {
            return;
    ...
    const onMouseOverDay = (ev)=>{
        ...
        dayRefs.forEach((dayRef, index)=>{
            if (dayRef) {
                ...
                if (!dayInfos[index].isSelected && dateRangeType === _index.DateRangeType.Day && daysToSelectInDayView && daysToSelectInDayView > 1) {
