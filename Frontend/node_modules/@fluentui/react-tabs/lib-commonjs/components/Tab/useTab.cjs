...
const _index = require("../TabList/index.cjs");
const useTab_unstable = (props, ref)=>{
    ...
    const appearance = (0, _index.useTabListContext_unstable)((ctx)=>ctx.appearance);
    const reserveSelectedTabSpace = (0, _index.useTabListContext_unstable)((ctx)=>ctx.reserveSelectedTabSpace);
    const size = (0, _index.useTabListContext_unstable)((ctx)=>{
        var _ctx_size;
...
const useTabBase_unstable = (props, ref)=>{
    ...
    const selectTabOnFocus = (0, _index.useTabListContext_unstable)((ctx)=>ctx.selectTabOnFocus);
    const listDisabled = (0, _index.useTabListContext_unstable)((ctx)=>ctx.disabled);
    const selected = (0, _index.useTabListContext_unstable)((ctx)=>ctx.selectedValue === value);
    const onRegister = (0, _index.useTabListContext_unstable)((ctx)=>ctx.onRegister);
    const onUnregister = (0, _index.useTabListContext_unstable)((ctx)=>ctx.onUnregister);
    const onSelect = (0, _index.useTabListContext_unstable)((ctx)=>ctx.onSelect);
    const vertical = (0, _index.useTabListContext_unstable)((ctx)=>!!ctx.vertical);
