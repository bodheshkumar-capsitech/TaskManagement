...
const _useDisableBodyScroll = require("../../utils/useDisableBodyScroll.cjs");
const _DialogBackdropMotion = require("../DialogBackdropMotion.cjs");
const useDialogSurface_unstable = (props, ref)=>{
    const contextRef = (0, _reactmotion.useMotionForwardedRef)();
    const modalType = (0, _index.useDialogContext_unstable)((ctx)=>ctx.modalType);
    const isNestedDialog = (0, _index.useDialogContext_unstable)((ctx)=>ctx.isNestedDialog);
    const backdropOverride = (0, _index.useDialogBackdropContext_unstable)();
    const treatBackdropAsNested = backdropOverride !== null && backdropOverride !== void 0 ? backdropOverride : isNestedDialog;
    const modalAttributes = (0, _index.useDialogContext_unstable)((ctx)=>ctx.modalAttributes);
    const dialogRef = (0, _index.useDialogContext_unstable)((ctx)=>ctx.dialogRef);
    const requestOpenChange = (0, _index.useDialogContext_unstable)((ctx)=>ctx.requestOpenChange);
    const dialogTitleID = (0, _index.useDialogContext_unstable)((ctx)=>ctx.dialogTitleId);
    const open = (0, _index.useDialogContext_unstable)((ctx)=>ctx.open);
    const unmountOnClose = (0, _index.useDialogContext_unstable)((ctx)=>ctx.unmountOnClose);
