...
namespace Projects.Controllers
{
    ...
    public class AuthController : ApiControllerBase
    {
        ...
        public AuthController(ILogger<AuthController> logger, IEmailSender emailSender, UserManager<ApplicationUser> userManager,
          SignInManager<ApplicationUser> signInManager, DBConfiguration dbConfig,MongoDbContext context, IProjectService service) : base(dbConfig)
        {
            _logger = logger;
        ...
        public async Task<ApiResponse<UserLogInResponse>> Login([FromBody] UserLogInRequest model)
        {
            ...
            try
            {
                if (ValidateModel(response))
                {
                    ...
                    //    await _userManager.CreateAsync(admin, "welcome");
                    //}
        ...
        public async Task<ApiResponse<UserLogInResponse>> Refresh()
        {
            ...
            try
            {
                ...
                string newAccessToken = ((ApplicationSignInManager)_signInManager).GenerateJwtToken(user);
            ...
            catch (Exception ex)
            {
                ...
                response.Status = false;
                response.Message = "Unable to refresh access token.";
                response.Result = null;
                return response;
            }
        }
 ...
 /*       [Authorize(Roles = "ADMIN, HRMANAGER, HREXECUTIVE, EMPLOYEE, CANDIDATE")]*/
        ...
        #region DeleteUser
        [HttpPost("Deleteuser")]
        public async Task<ApiResponse<bool>> DeleteUser(CancellationToken cancellation)
        {
            var res = new ApiResponse<bool>();
            var userId = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
                if(userId == null)
                {
                    res.Message = "Userid is null,failed to delete";
                    res.Status = false;
                    res.Result = false;
                    return res;
                }
            ...
            try
            {
                    ...
                    await _service.DeleteAllByUserAsync(userId, cancellation);
